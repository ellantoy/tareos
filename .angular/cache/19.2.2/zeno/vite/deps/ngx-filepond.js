...
var createRoute = (routes, fn2) => ({
  ...
  actions2.filter((action) => routes[action.type]).forEach((action) => routes[action.type]({
    root: root2,
    props,
    action: action.data,
    timestamp,
...
var getDateString = (date = /* @__PURE__ */ new Date()) => `${date.getFullYear()}-${leftPad(date.getMonth() + 1, "00")}-${leftPad(date.getDate(), "00")}_${leftPad(date.getHours(), "00")}-${leftPad(date.getMinutes(), "00")}-${leftPad(date.getSeconds(), "00")}`;
...
var createItem = (origin = null, serverFileReference = null, file2 = null) => {
  ...
  const api = __spreadProps(__spreadValues({
    ...
    serverId: {
      get: () => state2.serverFileReference
    },
    transferId: {
      get: () => state2.transferId
    },
    status: {
      get: () => state2.status
    },
    filename: {
      get: () => state2.file.name
    },
...
var getItemByQueryFromState = (state2, itemHandler) => (_a = {}) => {
  var _b = _a, {
    query,
    success = () => {
    },
...
var actions = (dispatch, query, state2) => ({
  ...
  DID_SET_FILES: ({
    ...
    files.forEach((file2, index) => {
      if (activeItems.find((item2) => item2.source === file2.source || item2.file === file2.source)) return;
...
var fileInfo = createView({
  ...
  didCreateView: (root2) => {
    applyFilters("CREATE_VIEW", __spreadProps(__spreadValues({}, root2), {
      view: root2
    }));
  },
  create: create$2,
...
var write$2 = ({
  ...
  root2.ref.activeStyles.forEach(({
    control,
    key,
    value
  }) => {
...
var create$7 = ({
  ...
  root2.ref.handleClick = (e) => root2.dispatch("DID_ACTIVATE_ITEM", {
    id: props.id
  });
...
var dragItem = ({
  ...
  const item2 = root2.query("GET_ITEM", {
    id
  });
  const view = root2.childViews.find((child) => child.id === id);
...
var toggleAllowMultiple = ({
  root: root2,
  action
}) => {
  attrToggle(root2.element, "multiple", action.value);
};
var toggleDirectoryFilter = ({
  root: root2,
  action
}) => {
...
var create$e = ({
  ...
  root2.ref.data = root2.appendChildView(root2.createChildView(data, __spreadValues({}, props)));
  ...
  root2.query("GET_STYLES").filter((style) => !isEmpty(style.value)).map(({
    name: name2,
    value
  }) => {
...
var write$9 = ({
  ...
  actions2.filter((action) => /^DID_SET_STYLE_/.test(action.type)).filter((action) => !isEmpty(action.data.value)).map(({
    type,
    data: data2
  }) => {
...
var createApp = (initialOptions = {}) => {
  ...
  const setOptions2 = (options) => store.dispatch("SET_OPTIONS", {
    options
  });
  ...
  const removeFile = (query, options) => {
    ...
    store.dispatch("REMOVE_ITEM", __spreadProps(__spreadValues({}, options), {
      query
    }));
...
if (supported()) {
  ...
  Status$1 = __spreadValues({}, Status);
  FileOrigin$1 = __spreadValues({}, FileOrigin);
  FileStatus = __spreadValues({}, ItemStatus);
  OptionTypes = {};
  updateOptionTypes();
  create$f = (...args) => {
...
var FilePondComponent = class {
  ...
  ngAfterViewInit() {
    ...
    inputAttributes.forEach((name2) => {
      const value = attributes[name2] ? attributes[name2].value : this.options[name2];
      if (!value) {
        return;
      }
    ...
    this.zone.runOutsideAngular(() => {
      this.pond = create$f(input, __spreadProps(__spreadValues({}, this.options), {
        // our initial files
        files: this.files
      }));
    });
...
FilePondComponent.ɵcmp = ɵɵdefineComponent({
...
FilePondModule.ɵmod = ɵɵdefineNgModule({
  type: FilePondModule,
  declarations: [FilePondComponent],
  imports: [CommonModule],
  exports: [FilePondComponent]
});
FilePondModule.ɵinj = ɵɵdefineInjector({
  imports: [CommonModule]
});
(() => {
