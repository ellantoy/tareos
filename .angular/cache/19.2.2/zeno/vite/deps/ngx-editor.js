  ...
  */
  findIndex(pos, round2 = -1) {
    ...
    for (let i = 0, curPos = 0; ; i++) {
      ...
      if (end >= pos) {
        if (end == pos || round2 > 0) return retIndex(i + 1, end);
...
var ParseContext = class {
  ...
  addTextNode(dom, marks2) {
    ...
    if (preserveWS === "full" || top.inlineContext(dom) || /[^ \t\r\n\u000c]/.test(value)) {
      ...
      if (value) this.insertNode(this.parser.schema.text(value), marks2);
      this.findInText(dom);
    } else {
      this.findInside(dom);
    }
  ...
  ignoreFallback(dom, marks2) {
    if (dom.nodeName == "BR" && (!this.top.type || !this.top.type.inlineContent)) this.findPlace(this.parser.schema.text("-"), marks2);
  }
  ...
  addElementByRule(dom, rule, marks2, continueAfter) {
    ...
    if (rule.node) {
      ...
      } else if (!this.insertNode(nodeType.create(rule.attrs), marks2)) {
        this.leafFallback(dom, marks2);
      }
    } else {
    ...
    if (nodeType && nodeType.isLeaf) {
      ...
      this.findInside(dom);
      rule.getContent(dom, this.parser.schema).forEach((node) => this.insertNode(node, marks2));
    } else {
      let contentDOM = dom;
  ...
  enter(type, attrs, marks2, preserveWS) {
    let innerMarks = this.findPlace(type.create(attrs), marks2);
...
editHandlers.keypress = (view, _event) => {
  ...
  if (!(sel instanceof TextSelection) || !sel.$from.sameParent(sel.$to)) {
    ...
    if (!/[\r\n]/.test(text2) && !view.someProp("handleTextInput", (f) => f(view, sel.$from.pos, sel.$to.pos, text2))) view.dispatch(view.state.tr.insertText(text2).scrollIntoView());
    event.preventDefault();
  }
};
...
function readDOMChange(view, from2, to, typeOver, addedNodes) {
  ...
  if (inlineChange) {
    if ($from.pos == $to.pos) {
      ...
      if (view.someProp("handleTextInput", (f) => f(view, chFrom, chTo, text2))) return;
