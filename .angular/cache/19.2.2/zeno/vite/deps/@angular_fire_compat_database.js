import {
  ...
  USE_EMULATOR,
  ɵauthFactory
} from "./chunk-MTVLHTAN.js";
import "./chunk-Q4Z6JNRS.js";
import {
  FIREBASE_APP_NAME,
  FIREBASE_OPTIONS,
  VERSION,
...
var Tree = class {
   ...
   */
  constructor(name3 = "", parent = null, node = {
    children: {},
    childCount: 0
  }) {
    this.name = name3;
...
function createListReference(query2, afDatabase, injector) {
  const outsideAngularScheduler = afDatabase.schedulers.outsideAngular;
  const refInZone = inject(NgZone).run(() => query2.ref);
  return {
    query: query2,
    ...
    snapshotChanges(events) {
      return snapshotChanges(query2, events, outsideAngularScheduler).pipe(pendingUntilEvent(injector));
    },
    stateChanges(events) {
      return stateChanges(query2, events, outsideAngularScheduler).pipe(pendingUntilEvent(injector));
    },
    auditTrail(events) {
      return auditTrail(query2, events, outsideAngularScheduler).pipe(pendingUntilEvent(injector));
    },
...
function createObjectReference(query2, afDatabase, injector) {
  return {
    query: query2,
    snapshotChanges() {
      return createObjectSnapshotChanges(query2, afDatabase.schedulers.outsideAngular)().pipe(pendingUntilEvent(injector));
    },
    update(data) {
...
var AngularFireDatabase = class _AngularFireDatabase {
  ...
  list(pathOrRef, queryFn) {
    const ref2 = inject(NgZone).runOutsideAngular(() => getRef(this.database, pathOrRef));
    let query2 = ref2;
    if (queryFn) {
      query2 = queryFn(ref2);
    }
    return createListReference(query2, this, this.injector);
  }
  object(pathOrRef) {
    const ref2 = inject(NgZone).runOutsideAngular(() => getRef(this.database, pathOrRef));
    return createObjectReference(ref2, this, this.injector);
  }
  createPushId() {
    const ref2 = inject(NgZone).runOutsideAngular(() => this.database.ref());
